...
<html>
	...
	<div class="search-title">
		...
		<div class="description">
			<a
				href="${pageContext.request.contextPath}/admin/shangpinmanager.do?actiontype=load&seedid=202">发布商品</a>
		</div>
	</div>
	<!-- 搜索控件开始 -->
		...
		<tbody>
			...
			<c:forEach var="temshangpin" items="${listshangpin}">
				<tr>
					...
					<td><fmt:formatNumber value="${temshangpin.hyjia/temshangpin.jiage}" maxFractionDigits="2" pattern="0.00"/>
					    折
					</td>
					<td>${temshangpin.dazhe==1?"打折":"不打折"}</td>
					 <td>
					   <c:if test="${temshangpin.spstate==1}">
						   待审批
						</c:if>
						<c:if test="${temshangpin.spstate==2}">
					...
					<td>
						<c:if test="${temshangpin.spstate==1}">
						    ...
						    <a class="orange-href" href="${pageContext.request.contextPath}/admin/shangpinspnext.jsp?id=${temshangpin.id}">审批</a>
						</c:if>
						<a class="orange-href"
									href="${pageContext.request.contextPath}/admin/spcaigou.jsp?id=${temshangpin.id}">采购</a>
						<a class="orange-href"
						href="${pageContext.request.contextPath}/admin/shangpinmanager.do?actiontype=load&id=${temshangpin.id}">修改</a>
